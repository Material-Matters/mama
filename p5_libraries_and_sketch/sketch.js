...
function preload() {
  ...
  if (!isMobile) {
    for (let i = 0; i < imageFiles.length; i++) {
      images[i] = loadImage('p5_libraries_and_sketch/bouncingimage/' + imageFiles[i]);
    }
  } else {
...
function setup() {
  ...
  let cWidth = container ? container.clientWidth : windowWidth;
  let cHeight = container ? container.clientHeight : windowHeight;
